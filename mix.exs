defmodule WorkflowCmd.MixProject do
  ...
  defp aliases do
    [
      ...
      "release.win": ["cmd set MAKE=make", "cmd set CC=gcc", "cmd set MIX_ENV=prod", "cmd mix deps.get", "cmd mix release", "cmd copy _build\\prod\\rel\\bakeware\\#{@app}.exe .\\#{@app}-#{@version}.exe"], # release for win os
    ]
  end
