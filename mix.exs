defmodule WorkflowCmd.MixProject do
  ...
  defp aliases do
    [
      ...
      "release.win": ["cmd set MAKE=make", "cmd set CC=gcc", "cmd set MIX_ENV=prod", "cmd mix deps.get", "cmd mix release", "cmd zip -D .\\#{@app}-#{@version}.zip _build\\prod\\rel\\bakeware\\#{@app}.exe"], # release for win os
    ]
  end
